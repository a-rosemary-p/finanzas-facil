...
export function MovementDayGroup({
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      >
        <span className="text-sm font-bold" style={{ color: '#1A2B3A' }}>
          {formatDateWithWeekday(date)}
        </span>
        <span
          className="text-xs font-medium px-2 py-0.5 rounded-full ml-2 shrink-0 flex items-center gap-1"
      ...
      {/* Movimientos del día — animación max-height */}
      <div
        style={{
          overflow: 'hidden',
          maxHeight: expanded ? `${movements.length * 80}px` : '0',
