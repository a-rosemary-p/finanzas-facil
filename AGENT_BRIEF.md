...
>
> Nuevo: `app/manifest.ts` (PWA manifest); íconos maskable + 1024 para Play Store; eliminar cuenta (endpoint `/api/profile/delete` + UI doble-confirm + página pública `/eliminar-cuenta` para users sin acceso); `/.well-known/assetlinks.json` con SHA real; migración 028 `profiles.id ON DELETE CASCADE`; middleware actualizado con rutas públicas (`/manifest.webmanifest`, `/eliminar-cuenta`, `/.well-known/`). TWA Android build vive en `App Finanzas Pymes/android-keystore/` (out-of-repo) — keystore + .aab firmado + README con instrucciones de update futuro. Package: `mx.fiza.app`. SHA-256: `C9:3B:F6:A1:7D:4F:2E:4D:EE:D6:B0:5B:EA:C2:D9:CD:76:AD:44:10:9A:BA:5F:06:B5:01:54:A8:AD:57:4C:DD`.
>
> **v0.63 (heredado)**: cleanup completo de 14 TODOs (proyectos UX completion + bugs menores + nice-to-haves). Selector de proyecto en forms manuales /pendientes y /recurrentes. Ranking de proyectos en /reportes. 7 tarjetas de proyectos en /admin/analytics. ToastProvider + cleanup window.alert. UX hints. EditProjectModal confirm dirty. Migración 027 advisory_xact_lock.
>
...
>
> **v0.60 (heredado)**: `ProjectsOnboardingModal` rediseñado más visual (heroes con mocks CSS, texto reducido, dots clickeables). `PendientesHelpModal` nuevo accesible via botón "?" en `/pendientes`. Confirmación de seguridad URLs documentada.
>
> **v0.5 (heredado)**: Feature de Proyectos (Pro-only) — tabla `projects` (migration 024) + columnas `project_id` en `movements` y `recurring_movements`. 6 endpoints REST `/api/projects/*`. `/api/entry/confirm` y `/api/movements/[id]` aceptan projectId/projectCreateName/projectSuggestion. IA agresiva con `buildProjectsSection`. Páginas `/proyectos` (lista) y `/proyectos/[id]` (detalle). `<ProjectSelector>` en ConfirmationScreen. Toggle "Incluir archivados" en Ajustes. Nav entry "Proyectos" con candado para Free. Migraciones 024 (schema), 025 (UNIQUE active name), 026 (projects_onboarded_at). Eventos analytics: project_*, projects_teaser_clicked, projects_onboarding_completed.
...
- **Categorías (v0.4 — modelo nuevo)**: `CATEGORIES_MASTER` es el catálogo flat de 19 cats genéricas direction-neutra (la dirección ingreso/gasto la decide la IA por contexto). Cada user tiene su lista curada en `profiles.categories` (migration 023). `GIRO_DEFAULTS[giro]` es el subset pre-seleccionado al onboarding. Custom strings = **Pro only**, cap total 40. Los handlers de extracción (`/api/entry`, `/api/entry/photo`) leen `profile.categories` (fallback a `GIRO_DEFAULTS` → genérico) vía `getUserCategories()` en `lib/giro-categories.ts`. `isValidCategoryName()` relajado a "string 1-40 chars no-vacío" — la constraint real vive en UI + AI prompt + endpoint `/api/profile/categories` que valida la lista al guardar. **`GIRO_CATEGORIES` borrado en v0.4** — código viejo importando esa constante truena (usa `GIRO_DEFAULTS` o `CATEGORIES_MASTER`). La columna `movements.category` es TEXT libre desde migration 009 y acepta cualquier string que pase isValidCategoryName.
- **(v0.4) `<CategoryPicker>` + `<CategoryPickerModal>`** en `components/categories/`. Picker reusable en 4 surfaces: ProfilePromptModal step categories (inline), `/inicio` modal bloqueante (re-onboarding), `/ajustes` Card Categorías, `ConfirmationScreen` opción "+ Otra opción…". Modal con `onPick` set hace save+pick atómico antes de propagar al padre. Pro-gate custom dentro del componente con CTA a `startProCheckout()`.
- **(v0.4) AI prompt** ahora pasa lista flat con regla "dirección-neutra" + "no doble-describir tipo en description". Si tocas `lib/ai/prompts.ts`, ojo con backticks en los template literals (rompen el build — usa comillas simples o dobles).
